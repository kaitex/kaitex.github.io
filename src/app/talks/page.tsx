...
export default function BlogPage() {
  ...
  return (
    <div className="mt-15  max-w-4xl mx-auto">
      <div className=" text-3xl font-medium mb-4">Talks<span className="text-[#fe8019]">.</span> </div>
      <ul className="space-y-4">
        {posts.map((post) => (
          <li className="flex flex-col  gap-3 md:flex-row md:gap-5 text-lg" key={post.frontmatter.slug}>
            
            <span className="text-[#999]">{post.frontmatter.date}</span><Link className="linkWithUnderline" href={`/talks/${post.frontmatter.slug}`}>  <span className="text-[var(--accent)]">#</span> <span className="">{post.frontmatter.title}</span>
